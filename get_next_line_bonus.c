...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/01 15:31:04 by hel-mefe          #+#    #+#             */
/*   Updated: 2021/12/01 15:31:11 by hel-mefe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, buffer, 0) < 0 || fd >= FD_LIMIT)
		return (NULL);
	line = ft_treat_all_cases(fd, buffer[fd]);
	if (!line)
		return (NULL);
	return (line);
}
